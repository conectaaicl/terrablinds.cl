...
exports.createPayment = async (req, res, next) => {
    try {
        ...
        const params = {
            ...
            urlReturn: `${frontendUrl}/payment/result?quote=${quote.id}`
        };
    ...
    try {
        ...
        if (paymentData.commerceOrder) {
            ...
            if (quoteId) {
                const quote = await Quote.findByPk(quoteId);
                if (quote) {
                    const paid = paymentData.status === 2;
                    const newStatus = paid ? 'accepted' : 'pending';

                    // Only update and send emails if newly confirmed
                    if (paid && quote.status !== 'accepted' && quote.status !== 'completed') {
                        await quote.update({ status: newStatus });
                        console.log(`Quote #${quoteId} payment confirmed via Flow`);

                        // Send confirmation emails (non-blocking)
                        Promise.allSettled([
                            emailService.sendPaymentConfirmationEmail(quote, 'flow'),
                            emailService.sendAdminPaymentNotification(quote, 'flow'),
                        ]).catch(err => console.error('Email error after Flow payment:', err.message));
                    } else if (!paid) {
                        await quote.update({ status: newStatus });
                    }
                }

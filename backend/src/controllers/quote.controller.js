...
exports.createQuote = async (req, res) => {
    try {
        const { customer_name, customer_email, customer_phone, customer_address, customer_commune, customer_rut, notes, items } = req.body;
        ...
        // Generate TB-YYYY-NNNN quote number
        const year = new Date().getFullYear();
        const lastQuote = await Quote.findOne({ order: [['id', 'DESC']] });
        const seq = String((lastQuote ? lastQuote.id + 1 : 1)).padStart(4, '0');
        const quoteNum = `TB-${year}-${seq}`;
        ...
        const quote = await Quote.create({
            ...
            customer_commune: customer_commune ? String(customer_commune).substring(0, 200) : null,
            customer_rut: customer_rut ? String(customer_rut).substring(0, 20) : null,

        ...
        if (externalRef && externalRef.startsWith('TB-')) {
            ...
            if (quoteId) {
                ...
                if (quote) {
                    ...
                    if (approved && quote.status !== 'accepted' && quote.status !== 'completed') {
                        await quote.update({ status: 'accepted' });
                        console.log(`[MercadoPago] Quote #${quoteId} payment confirmed`);

                        // Send confirmation emails (non-blocking)
                        Promise.allSettled([
                            emailService.sendPaymentConfirmationEmail(quote, 'mp'),
                            emailService.sendAdminPaymentNotification(quote, 'mp'),
                        ]).catch(err => console.error('Email error after MP payment:', err.message));
                    } else if (!approved) {

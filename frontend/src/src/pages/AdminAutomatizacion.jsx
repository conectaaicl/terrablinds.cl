...
import { Save, Loader, CheckCircle, Zap, Upload, X } from 'lucide-react';
import api from '../api';
    ...
    return (
        <AdminLayout>
            ...
            <form onSubmit={save} className="space-y-5">
                ...
                <Section title="Fotos de la página" desc="Sube hasta 4 fotos que aparecerán en la galería de la página de automatización">
                    <div className="grid grid-cols-2 gap-4">
                        {[1, 2, 3, 4].map(i => (
                            <PhotoUpload key={i} label={`Foto ${i}`} name={`auto_photo${i}`} value={s[`auto_photo${i}`] || ''} onChange={onChange} />
                        ))}
                    </div>
                </Section>

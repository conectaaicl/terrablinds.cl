...
import { Save, Loader, CheckCircle, KeyRound, Upload, X } from 'lucide-react';
import api from '../api';
...
const DEFAULTS = {
    acceso_title: 'Control de Acceso — Cerraduras Electrónicas y Biometría',
    acceso_subtitle: 'Controla quién entra y cuándo. Sistemas de acceso biométrico, tarjetas RFID y apertura por smartphone para hogares y empresas.',
    acceso_feat1_title: 'Huella, tarjeta o PIN',
    acceso_feat1_desc: 'Múltiples métodos de autenticación. Sin llaves físicas que perder o copiar.',
    acceso_feat2_title: 'Apertura por app',
    acceso_feat2_desc: 'Abre puertas y portones desde tu smartphone, en cualquier lugar del mundo.',
    acceso_feat3_title: 'Gestión de usuarios',
    acceso_feat3_desc: 'Agrega, edita y elimina usuarios. Define horarios de acceso por persona.',
    acceso_feat4_title: 'Registro de accesos',
    acceso_feat4_desc: 'Historial completo de quién entró, cuándo y por qué punto de acceso.',
    acceso_feat5_title: 'Anti-sabotaje',
    acceso_feat5_desc: 'Alarma ante intentos forzados. Bloqueo automático tras varios intentos fallidos.',
    acceso_feat6_title: 'Instalación integral',
    acceso_feat6_desc: 'Instalamos cerradura, lector, cableado y configuramos el sistema completo.',
        ...
        <AdminLayout>
            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                    <KeyRound className="w-6 h-6 text-blue-700" /> Control de Acceso
                </h1>
                <p className="text-gray-500 text-sm mt-0.5">
                    Edita el contenido de la página de control de acceso.{' '}
                    <a href="/control-acceso" target="_blank" className="text-blue-600 hover:underline">Ver página →</a>
                    {' · '}
            ...
            <form onSubmit={save} className="space-y-5">
                ...
                <Section title="Encabezado (Hero)" desc="Título y subtítulo de la página de control de acceso">
                    <div className="space-y-4">
                        <Field label="Título principal">
                            <input className={INPUT} name="acceso_title" value={s.acceso_title || ''} onChange={onChange} />
                        </Field>
                        <Field label="Subtítulo">
                            <textarea rows={2} className={TEXTAREA} name="acceso_subtitle" value={s.acceso_subtitle || ''} onChange={onChange} />
                        </Field>
                    </div>
                </Section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[1, 2, 3, 4, 5, 6].map(i => (
                            <div key={i} className="bg-gray-50 rounded-lg p-4 space-y-2">
                                ...
                                <Field label="Título">
                                    <input className={INPUT} name={`acceso_feat${i}_title`} value={s[`acceso_feat${i}_title`] || ''} onChange={onChange} />
                                </Field>
                                <Field label="Descripción">
                                    <input className={INPUT} name={`acceso_feat${i}_desc`} value={s[`acceso_feat${i}_desc`] || ''} onChange={onChange} />
                                </Field>
                            </div>
                        ))}
                    </div>
                ...
                <Section title="Fotos de la página" desc="Sube hasta 4 fotos que aparecerán en la galería de instalaciones">
                    <div className="grid grid-cols-2 gap-4">
                        {[1, 2, 3, 4].map(i => (
                            <PhotoUpload key={i} label={`Foto ${i}`} name={`acceso_photo${i}`} value={s[`acceso_photo${i}`] || ''} onChange={onChange} />
                        ))}
                    </div>
                </Section>

                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-700">
                    <strong>Productos de control de acceso:</strong> Se gestionan desde{' '}
                    <Link to="/admin/products" className="underline font-semibold">Admin → Productos</Link>.
                    Usa las categorías <strong>"Control de Acceso"</strong>, <strong>"Cerradura Electrónica"</strong> o{' '}
                    <strong>"Portero Eléctrico"</strong> para que aparezcan en esta página.
                </div>

...
export default function AdminBlog() {
    ...
    const load = () => {
        setLoading(true);
        api.get('/api/blog/admin/all').then(r => setPosts(r.data)).catch(() => setPosts([])).finally(() => setLoading(false));
    };
    ...
    const save = async (e) => {
        ...
        try {
            if (editing === 'new') {
                ...
                await api.put(`/api/blog/${editing}`, form);
            }
            setSaved(true);
    ...
    const deletePost = async (id) => {
        ...
        setDeleting(id);
        try { await api.delete(`/api/blog/${id}`); load(); }
        catch { alert('Error al eliminar.'); }
        finally { setDeleting(null); }
    };

    const togglePublish = async (post) => {
        try { await api.put(`/api/blog/${post.id}`, { is_published: !post.is_published }); load(); }

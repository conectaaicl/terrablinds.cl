...
export default function BookingResult() {
    ...
    useEffect(() => {
        if (isFree && bookingId) {
            // Free service confirmed directly
            api.get('/api/bookings/resultado?token=free&id=' + bookingId)
               .then(r => { setData({ ...r.data, status: 'confirmed' }); })
               .catch(() => setData({ status: 'confirmed', client_name: '' }))
               .finally(() => setLoading(false));
            return;
        }
        if (isPending) {
            setData({ status: 'pending_wa', waUrl });
            setLoading(false);
            return;
        }
    ...
    const pendingWa = data?.status === 'pending_wa';
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
                <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center space-y-6">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">¡Reserva recibida!</h1>
                        <p className="text-gray-500 text-sm mt-2">Tu solicitud fue registrada. Coordina el pago por WhatsApp para confirmar tu hora.</p>
                    </div>
                    {data?.waUrl && (
                        <a href={data.waUrl} target="_blank" rel="noopener noreferrer"
                            className="block w-full py-3 bg-green-500 text-white rounded-xl text-sm font-semibold hover:bg-green-600 transition-colors">
                            💬 Confirmar por WhatsApp
                        </a>
                    )}
                    <Link to="/" className="block text-sm text-gray-400 hover:text-gray-600">Volver al inicio</Link>
                </div>
            </div>
        );
    }

                ...
                <div className="fixed bottom-24 right-5 z-50 w-[340px] sm:w-[380px] flex flex-col rounded-2xl shadow-2xl border border-gray-200 bg-white overflow-hidden"
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 bg-[#0d3a8a] flex-shrink-0">
                        ...
                        <div className="flex items-center gap-1">
                            {waNumber && (
                                <a href={`https://wa.me/${waNumber}?text=${encodeURIComponent('Hola TerraBlinds, necesito ayuda con una cotización.')}`}
                                    target="_blank" rel="noopener noreferrer"
                                    title="Hablar con un asesor por WhatsApp"
                                    className="p-1.5 rounded-lg text-green-300 hover:text-green-200 hover:bg-white/10 transition-colors flex items-center">
                        ...
                        {messages.length >= 3 && messages[messages.length - 1]?.role === 'assistant' && !loading && (
                            <div className="flex flex-wrap justify-center gap-2">
                                <Link to="/quote" onClick={() => setOpen(false)}
                                    ...
                                    📋 Cotizador <ChevronRight className="w-3 h-3" />
                                </Link>
                                {waNumber && (
                                    <a href={`https://wa.me/${waNumber}?text=${encodeURIComponent('Hola, estaba chateando en su web y me gustaría continuar por WhatsApp.')}`}
                                        target="_blank" rel="noopener noreferrer"

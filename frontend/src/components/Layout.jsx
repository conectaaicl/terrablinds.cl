...
const Layout = ({ children }) => {
    ...
    return (
        <div className="min-h-screen bg-white flex flex-col font-sans">
            ...
            {/* ── Navbar ── */}
            <header className="sticky top-0 z-50 bg-[#1a56db] shadow-lg shadow-blue-900/30">
                <div className="container mx-auto px-4 py-3 flex justify-between items-center">
                    ...
                    <nav className="hidden lg:flex items-center space-x-0.5 text-sm font-semibold">
                        {[
                            ...
                            <Link key={to} to={to}
                                className="relative px-3 py-2 rounded-lg text-white/85 hover:bg-white/15 hover:text-white font-semibold transition-all duration-200 group">
                                {label}
                                <span className="absolute bottom-0.5 left-3 right-3 h-0.5 bg-white/70 scale-x-0 group-hover:scale-x-100 transition-transform duration-200 rounded-full" />
                            </Link>
                        ))}

                        {/* Servicios dropdown */}
                        <div className="relative group/services">
                            <button className="relative px-3 py-2 rounded-lg text-white/85 hover:bg-white/15 hover:text-white font-semibold transition-all duration-200 flex items-center gap-1">
                                Servicios
                                <svg className="w-3.5 h-3.5 opacity-70 group-hover/services:rotate-180 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                            </button>
                            <div className="absolute top-full left-0 mt-1 w-52 bg-white rounded-xl shadow-xl shadow-black/20 border border-gray-100 overflow-hidden opacity-0 invisible group-hover/services:opacity-100 group-hover/services:visible transition-all duration-200 z-50">
                                {[
                                    { to: '/software', label: '💻 Software' },
                                    { to: '/domotica', label: '🏠 Domótica' },
                                    { to: '/cortinas-metalicas', label: '⚙️ Cortinas Metálicas' },
                                    { to: '/automatizacion', label: '🤖 Automatización' },
                                    { to: '/camaras', label: '📷 Cámaras de Seguridad' },
                                    { to: '/paneles-solares', label: '☀️ Paneles Solares' },
                                    { to: '/control-acceso', label: '🔒 Control de Acceso' },
                                ].map(({ to, label }) => (
                                    <Link key={to} to={to}
                                        className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50 hover:text-[#1a56db] font-medium transition-colors">
                                        {label}
                                    </Link>
                                ))}
                            </div>
                    ...
                    <div className="hidden lg:flex items-center space-x-3">
                        ...
                        <Link to="/quote"
                            className="px-4 py-2 bg-white hover:bg-blue-50 text-[#1a56db] text-sm font-semibold rounded-lg transition-colors shadow-md">
                            Cotizar
                        </Link>
                    </div>
                ...
                {/* Mobile menu */}
                {isMenuOpen && (
                    <div className="lg:hidden bg-[#1545b8] border-t border-white/20">
